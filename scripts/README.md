...
- `usr/bin/png2jpg.sh` provides local conversion modes: dry run (`0`), conversion (`1`), transactional pull/missing-JPG generation/validation/commit/push (`2`), and non-pushing conversion plus validation (`3`). It only manages JPG files; profile README and CSV metadata are generated by the Node workflow. Mode 2 preserves JPGs and profile data already present after the pull and generates only missing fallbacks.
