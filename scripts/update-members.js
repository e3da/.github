...
// CSV and README. Raw PNGs are referenced directly; raw JPEGs use generated
// 64x64 JPG fallbacks.
...
function resolveProfilePath(relativePath) {
  const absolutePath = path.resolve(profileDirectory, relativePath);
  const fromProfile = path.relative(profileDirectory, absolutePath);
  if (fromProfile === '..' || fromProfile.startsWith(`..${path.sep}`) || path.isAbsolute(fromProfile)) {
    throw new Error(`Profile path escapes the profile directory: ${relativePath}`);
  }
  return absolutePath;
}
...
async function findAvatarPath(directory, login, extension) {
  ...
  const matches = names
    .filter((entry) => entry.isFile() && entry.name.toLowerCase() === normalizedName)
...
async function rawAvatarPaths(login) {
  let names;
  try {
    names = await fs.readdir(rawAvatarDirectory, { withFileTypes: true });
  } catch (error) {
    if (error.code === 'ENOENT') return [];
    throw error;
  }
  ...
  return names
    ...
    .map((entry) => path.join(rawAvatarDirectory, entry.name));
}

async function assertUniqueRawAvatars() {
  const entries = await fs.readdir(rawAvatarDirectory, { withFileTypes: true });
  const seen = new Map();
...
async function downloadAvatar(user) {
  ...
  try {
    ...
    temporaryPath = `${rawPath}.${process.pid}.${randomUUID()}.tmp`;
...
async function avatarPath(login) {
  ...
  const rawJpgPath = await findAvatarPath(rawAvatarDirectory, login, '.jpg') ||
    await findAvatarPath(rawAvatarDirectory, login, '.jpeg');
  if (rawJpgPath) {
...
async function main() {
  ...
  for (const profile of profiles.values()) {
    if (!activeLogins.has(profile.username)) {
      ...
      profile.avatar = '';
      if (previousAvatar) {
        await fs.rm(resolveProfilePath(previousAvatar), { force: true });
      }
      for (const rawPath of await rawAvatarPaths(profile.username)) {
        await fs.rm(rawPath, { force: true });
      }
      const jpgEntries = await fs.readdir(jpgAvatarDirectory, { withFileTypes: true }).catch((error) => {
        if (error.code === 'ENOENT') return [];
        throw error;
      });
      for (const entry of jpgEntries) {
        const extension = path.extname(entry.name).toLowerCase();
        const basename = path.basename(entry.name, path.extname(entry.name)).toLowerCase();
        if (entry.isFile() && basename === profile.username && ['.jpg', '.jpeg'].includes(extension)) {
          await fs.rm(path.join(jpgAvatarDirectory, entry.name), { force: true });
        }
      }
    }
  }
  ...
  await writeFileAtomically(csvPath, csvContent);
  ...
  await writeFileAtomically(readmePath, newContent);
}
